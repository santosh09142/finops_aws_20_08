...
class AWSServiceRunner:
    ...
    def __init__(self, base_session, connector, region, services, accounts,
                 role_name):
        ...
        # Ensure SERVICE_MAP entries are actual callables (classes), not
        # strings. config.SERVICE_MAP may contain class names as strings;
        # replace them with the module-level imported classes (S3Service,
        # EC2Service, ...)
        self.SERVICE_MAP = {}
        for name, val in config.SERVICE_MAP.items():
            resolved = None
            if isinstance(val, str):
                # support module-qualified string like
                # 'core.s3_service:S3Service'
                if ':' in val:
                    ...
                    try:
                        ...
                        if resolved is None:
                            logger.warning("Class %s not found in module %s "
                                           "for service %s", cls_name,
                                           module_path, name)
                    except (ImportError, AttributeError) as e:
                        logger.exception("Failed importing %s for service "
                                         "%s: %s", val, name, e)
                else:
                    # fallback: try resolving simple class name from globals
                    resolved = globals().get(val)
                    if resolved is None:
                        logger.warning("Service class '%s' for key '%s' not "
                                       "found in globals", val, name)
            else:
                resolved = val
